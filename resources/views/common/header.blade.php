...
=======
      ...
      <div class="rounded-3xl bg-white shadow-lg">
        <div class="px-4 py-6 sm:px-8 sm:py-10">
            ...
            <ul class="-my-8">
              <li class="flex flex-col space-y-3 py-6 text-left sm:flex-row sm:space-x-5 sm:space-y-0">
                <div class="shrink-0 relative">
                  <span class="absolute top-1 left-1 flex h-6 w-6 items-center justify-center rounded-full border bg-white text-sm font-medium text-gray-500 shadow sm:-top-2 sm:-right-2">2</span>
                  <img class="h-24 w-24 max-w-full rounded-lg object-cover" src="https://images.unsplash.com/photo-1588484628369-dd7a85bfdc38?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8MTh8fHNuZWFrZXJ8ZW58MHx8MHx8&auto=format&fit=crop&w=150&q=60" alt="Product Image 1" />
                </div>
          ...
          <div class="mt-6 text-center">
            <button type="button" class="group inline-flex w-full items-center justify-center rounded-md bg-yellow-300 px-6 py-4 text-lg font-semibold ">
              Place Order
            </button>
          </div>
        </div>
  ...
  {{-- LOGIN :: USER PROFILE --}}
  @if (!Auth::user())
    <button class="btn ml-3" onclick="window.location.href='{{ url('login') }}'">LOGIN</button>
  @else
>>>>>>> Stashed changes
  {{-- LOGIN :: USER PROFILE --}}
  @if (!Auth::user())
